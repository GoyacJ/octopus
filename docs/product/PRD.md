...
| 工具可见性与结构化交互不可解释 | 用 CapabilityCatalog、CapabilityResolver、ToolSearch、InteractionPrompt 与 MessageDraft 建立正式 capability runtime |
...
4. **能力必须进入正式运行时**：工具是否存在、是否可见、是否只可搜索不可执行，以及是否需要结构化交互，都必须由正式上下文求值，而不是 prompt 层偶然行为。
5. **自治必须先有边界**：任何自治执行都建立在作用域、预算、时间窗、工具集、环境边界和撤销机制之上。
6. **知识是系统资产**：私有记忆、共享知识和组织知识图谱都必须具备来源、权限、晋升、删除和审计语义。
7. **协议优先于私有耦合**：对外工具接入以 MCP 为主，对外 Agent 协作以 A2A 为主，内部模型需与协议边界兼容。
8. **跨表面连续性**：桌面端、Web、移动端对同一 Hub 共享统一对象语义、统一状态机和统一通知语义。
9. **结果与过程都可治理**：Artifact、Trace、Audit、Policy Decision、Knowledge Lineage 都是一等对象。
...
- 用户和管理员可以解释为什么某个 capability 在当前上下文中可见、不可见、只可搜索不可执行，以及为什么系统发出了结构化澄清或消息草稿。
...
- 首轮 GA 的页面优先级、最小页面集合、信息架构层级与视觉语法应由 [`VISUAL_FRAMEWORK.md`](../architecture/VISUAL_FRAMEWORK.md) 补充定义。
...
- 工具发现、结构化交互与消息草稿的具体实现形态可参考 `docs/references/Claude_Hidden_Toolkit.md`，但该参考资料只用于借鉴工具分层与交互模式，不改变本 PRD 定义的正式对象边界与产品范围
