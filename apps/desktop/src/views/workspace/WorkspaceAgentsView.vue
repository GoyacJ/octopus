...
watch(
  () => agentStore.workspaceAgents.map(agent => agent.id).join('|'),
  () => {
    if (!selectedAgentId.value || !agentStore.workspaceAgents.some(agent => agent.id === selectedAgentId.value)) {
      applyAgent(agentStore.workspaceAgents[0]?.id)
      return
    }
...
const metrics = computed(() => [
  { id: 'total', label: t('agents.stats.total'), value: String(agentStore.workspaceAgents.length) },
  { id: 'active', label: t('agents.stats.active'), value: String(agentStore.workspaceAgents.filter(agent => agent.status === 'active').length) },
])

function applyAgent(agentId?: string) {
  const agent = agentStore.workspaceAgents.find(item => item.id === agentId)
...
<template>
  <div class="flex w-full flex-col gap-6 pb-20">
    <header class="space-y-4 px-2">
      <UiSectionHeading :eyebrow="t('agents.header.eyebrow')" :title="t('sidebar.navigation.agents')" :subtitle="agentStore.error || t('agents.workspace.descriptionAgent')" />
    ...
    <div class="grid gap-6 px-2 xl:grid-cols-[minmax(0,1fr)_360px]">
      <section class="space-y-3">
        <UiRecordCard
          v-for="agent in agentStore.workspaceAgents"
          :key="agent.id"
          :title="agent.name"
        ...
        <UiEmptyState v-if="!agentStore.workspaceAgents.length" :title="t('agents.empty.workspaceTitle')" :description="t('agents.empty.workspaceDescription')" />
      </section>
