...
const pageTitle = computed(() => isProjectScope.value
  ? workspaceStore.activeProject?.name ?? 'Project Agents'
  : t('sidebar.navigation.agents'))
const pageDescription = computed(() => isProjectScope.value
  ? workspaceStore.activeProject?.description ?? 'Manage project-owned agents and teams.'
  : 'Manage workspace agents and team relationships in one place.')

const centerStats = computed(() => [
  {
    label: '活跃员工',
    value: String(currentAgents.value.filter(a => a.status === 'active').length),
    helper: '当前可用 Agent 数量',
    tone: 'success' as const,
  },
  {
    label: '协作团队',
    value: String(currentTeams.value.length),
    helper: '已组建的 Agent 团队',
    tone: 'info' as const,
  },
  {
    label: '工具集成',
...
<template>
  <div class="flex h-full min-h-0 w-full flex-col gap-6 pb-20">
    <AgentsHeroSection
      ...
      :scope-label="props.scope === 'project' ? 'Project Scope' : 'Workspace Scope'"
      active-mode-label="Center"
    />
      ...
      <div v-if="teamTotal" :class="teamViewMode === 'card' ? 'grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4' : 'space-y-2'">
        <template v-for="team in pagedTeams" :key="team.id">
          <TeamUnitCard
            ...
            :lead-label="currentAgents.find(agent => agent.id === team.leaderAgentId)?.name ?? '未设置 leader'"
          ...
          >
            ...
            <div class="flex items-center gap-8 w-full overflow-hidden">
              <div class="flex min-w-0 flex-[2] flex-col gap-0.5">
                <span class="truncate text-[11px] font-bold uppercase tracking-wider text-indigo-600/70">{{ team.personality || 'Agent Team' }}</span>
                <p class="truncate text-sm text-text-secondary/80">
                  {{ team.description }}
                </p>
              </div>
              ...
              <div class="hidden shrink-0 items-center gap-6 md:flex">
                <div class="flex flex-col items-end">
                  <span class="text-[9px] font-bold uppercase tracking-tighter text-text-tertiary/40">Leader</span>
                  <span class="text-xs font-bold text-text-primary/70 truncate max-w-[80px]">
                    {{ currentAgents.find(agent => agent.id === team.leaderAgentId)?.name ?? 'N/A' }}
                  </span>
                </div>
                <div class="flex flex-col items-end">
    ...
    <UiDialog
      ...
      title="员工配置"
      description="配置 Agent 的基本信息、核心性格与工具能力。"
