...
impl Tool for AgentTool {
    ...
    async fn execute(&self, input: Value, ctx: &ToolContext) -> ToolResult {
        ...
        let query_config = build_subagent_query_config(model, system_prompt, params.max_turns);
...
fn build_subagent_query_config(
    model: String,
...
mod tests {
    ...
    #[test]
    fn test_build_subagent_query_config_uses_default_prompt_context() {
        let cfg = build_subagent_query_config(
            "claude-sonnet-4-6".to_string(),
            "sub-agent prompt".to_string(),
            Some(7),
        );

        assert_eq!(cfg.model, "claude-sonnet-4-6");
        assert_eq!(cfg.max_turns, 7);
        assert_eq!(cfg.system_prompt, Some("sub-agent prompt".to_string()));
        assert_eq!(
            cfg.output_style,
            cc_core::system_prompt::OutputStyle::Default
        );
