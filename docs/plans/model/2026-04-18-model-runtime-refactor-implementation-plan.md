...
- Verified in this worktree: `cargo test -p octopus-runtime-adapter --tests`, `cargo test -p octopus-server project_task_routes_approve_intervention_keeps_waiting_when_runtime_chains_to_next_approval` with provider env stubs, `pnpm schema:check`, `pnpm -C apps/desktop test`, and `pnpm -C apps/desktop typecheck`.
- Known unrelated fence: `pnpm check:frontend` is still blocked by a pre-existing frontend governance violation in `apps/desktop/src/views/project/ProjectSettingsView.vue`; this refactor did not introduce that failure.
