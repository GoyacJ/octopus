...
- **1.8.1（2026-04-25）**：架构评审 P0/P1/P2 修订（14 项），见 `docs/architecture/harness/audit/2026-04-25-architecture-review.md`。
  - **P0**（2 项）：①`overview.md §2 P6` + 新增 `security-trust.md §10.W` 明确 Hook FailOpen 是 P6 受控例外；②`permission-model.md §5.1` 的 `DirectBroker` 回调签名补 `PermissionContext` 参数，与 `harness-permission.md §3.1` 对齐。
  - **P1**（5 项）：①`module-boundaries.md §3.7` 新增"Feature 触发依赖附录"；②§10 例外登记表登记 3 条破窗（`auto-mode` / `redactor` / `subagent-tool`）；③`harness-contracts.md §3` 的 `EndReason` 新增 `Cancelled { initiator: CancelInitiator }` 变体（CancelInitiator: User / Parent / System），`harness-engine.md §5` 增加触发源映射表；④`harness-hook.md §2.6.2` 新增"Hook 失败的事务语义（all-or-nothing）"段；⑤`harness-model.md §2.4` 加强 `CredentialKey.tenant_id` 必填语义 + 新增 `TenantId::SHARED` 哨兵 + 新增 `Event::CredentialPoolSharedAcrossTenants` 必记事件。
  - **P2**（7 项）：①`api-contracts.md` 头注扩补 ADR-0009/0011/0015/0016/0017/0018，§14.3 新增 `EngineRunner` trait，§17.2/17.3 新增 `PluginManifestLoader / PluginRuntimeLoader` trait；②`adr/0011-tool-capability-handle.md §0` 新增"术语对齐"块，禁止 `ToolCapabilityHandle` / `ToolCap` 别名；③`harness-contracts.md` `ToolOrigin::Plugin.plugin_id` 改为 `PluginId` newtype；④`harness-sdk.md §8.1/8.2` 新增 Builder 幂等覆盖语义 + Session 不走 type-state 的设计声明；⑤新增 `Event::GraceCallTriggered` 变体（`event-schema.md §3.1.1`）+ `harness-engine.md §4.1` 同步发出契约；⑥`harness-tool.md §2.7` 显式声明"bool 二档分组（非三桶）"+ `overview.md §7.1` 流程图同步标注；⑦`harness-observability.md §2.5.0` 新增"Redactor 必经管道契约"+ `harness-journal.md §2.1` 头注同步声明 EventStore 实现必须装配 `Arc<dyn Redactor>`。
