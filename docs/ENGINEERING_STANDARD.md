...
### 7.7 AI 前端设计提示模板（默认参考）

为降低 AI 辅助前端实现时的视觉漂移，本节提供默认提示模板。

说明：

- 本节是给 AI 的默认提示模板，不单独构成超出本规范其他条款的新强制要求。
- 若与既有产品边界、信息架构、品牌约束、共享 design tokens、i18n、主题规范或已落地组件体系冲突，以正式文档和当前实现为准。
- 若页面已经存在稳定设计语言，AI SHOULD 优先延续，而不是为了“更好看”而重做整套视觉。

默认提示模板如下：

> 你现在是一位资深的前端设计助手。请在不改变信息架构、领域边界、交互主流程和正式契约的前提下，基于现代简约风格优化我的前端页面。我希望整体大气、克制、操作流畅，兼具 Apple 与 Google 设计中的秩序感、留白、层级与反馈节奏，但它们只作为气质参考，不直接模仿具体产品界面。请提出清晰、可实施的优化建议，并重点覆盖以下方面：
>
> 1. 色彩搭配：基于 shared design tokens 规划主色、强调色、中性色、背景层级和状态色，避免硬编码颜色；同时说明 `light / dark / system` 下的对比关系、可读性和层次变化。
> 2. 布局与留白：使用一致的栅格、对齐方式、区块节奏和卡片层级，优先采用 8px 倍数的间距体系，避免无规律堆叠与视觉拥挤。
> 3. 排版层级：明确标题、正文、说明、标签、辅助信息的字号、字重、行高和密度，让信息主次一眼可辨，并兼顾 `zh-CN` / `en-US` 两种语言长度差异。
> 4. 交互细节：补齐 `hover`、`focus`、`active`、`disabled`、`loading`、`empty`、`error`、`success` 等状态，确保反馈即时、过渡克制、动画短促且可预期，避免炫技式动效。
> 5. 组件细节：针对按钮、输入框、选择器、表格、卡片、导航、弹窗、抽屉、空态和错误态，说明如何通过圆角、边框、阴影、分隔、密度和反馈节奏提升现代感，并保持同一套视觉语言。
> 6. 视觉一致性：确保同一页面中颜色语义、圆角体系、阴影强度、边框表达、图标风格和交互反馈保持统一；新增视觉语义优先沉淀为 token 或公共 variant，而不是页面局部一次性样式。
> 7. 输出方式：建议必须具体到颜色角色、间距节奏、排版层级、组件状态和可执行调整项，避免只给抽象审美判断；如果发现现有设计系统已足够清晰，应优先做增量优化而不是整体推翻。
