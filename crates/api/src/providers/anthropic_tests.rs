use super::super::provider_errors::{
    is_retryable_status, ALT_REQUEST_ID_HEADER, REQUEST_ID_HEADER,
};
...
use super::anthropic_auth::{read_api_key, read_auth_token};
use super::anthropic_oauth::now_unix_timestamp;
use super::{
    oauth_token_is_expired, resolve_saved_oauth_token, resolve_startup_auth_source,
    AnthropicClient, AuthSource, OAuthTokenSet,
};
...
fn read_api_key_requires_presence() {
    ...
    let error = read_api_key().expect_err("missing key should error");
    assert!(matches!(
        error,
...
fn read_api_key_requires_non_empty_value() {
    ...
    std::env::remove_var("ANTHROPIC_API_KEY");
    let error = read_api_key().expect_err("empty key should error");
    assert!(matches!(
        error,
        crate::error::ApiError::MissingCredentials { .. }
    ));
...
fn read_api_key_prefers_api_key_env() {
    ...
    assert_eq!(read_api_key().expect("api key should load"), "legacy-key");
    std::env::remove_var("ANTHROPIC_AUTH_TOKEN");
    std::env::remove_var("ANTHROPIC_API_KEY");
}
...
fn read_auth_token_reads_auth_token_env() {
    let _guard = env_lock();
    std::env::set_var("ANTHROPIC_AUTH_TOKEN", "auth-token");
    assert_eq!(read_auth_token().as_deref(), Some("auth-token"));
    std::env::remove_var("ANTHROPIC_AUTH_TOKEN");
}
