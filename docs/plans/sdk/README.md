...
| `12-post-w8-capability-hardening.md` | Post-W8：收口 live runtime 仍暴露的 stub tools / stub-backed models / plugin live bootstrap，并在稳定后回填 contracts | `draft` |

> 本索引随周计划产出逐步更新 `状态`。预登记周计划可保持 `pending` 且暂未建文件；文件创建后按 `draft → in_progress → blocked/done` 流转。
>
> `12-post-w8-capability-hardening.md` 是 W8 收尾后的 follow-up tranche；`04`–`11` 的 `done` 状态保持不变。
...
**最后更新**：2026-04-23（新增 Post-W8 子 Plan `12-post-w8-capability-hardening.md`，当前状态 `draft`）
