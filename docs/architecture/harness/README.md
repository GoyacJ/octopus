...
> 版本：1.8（吸收 Claude Code / Hermes / OpenClaw 三端剩余优秀设计：引入 Programmatic Tool Calling `execute_code` 元工具 + Steering Queue 软引导 + 显式拒绝 Loop-Intercepted Tools；ToolResultPart 转为正向白名单）
