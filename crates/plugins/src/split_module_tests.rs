...
#[test]
fn split_module_roundtrip_manifest_loading_and_registry_behaviors() {
    let root = temp_dir("split-seam");
    ...
    assert_eq!(definition.metadata().id, "loader-demo@external");
    assert_eq!(definition.hooks().pre_tool_use.len(), 1);
    assert_eq!(definition.tools().len(), 1);

    let registry = PluginRegistry::new(vec![RegisteredPlugin::new(definition, true)]);
    let aggregated_tools = registry.aggregated_tools().expect("tools should aggregate");
    assert_eq!(aggregated_tools.len(), 1);
