...
- turn 执行时再把 `configured model` 解析成 `ResolvedExecutionTarget`，补齐 secret/base URL/max tokens，并根据 `executionProfile.executionClass` 进入 `agent_conversation` 或 `single_shot_generation` 执行路径。
- agent session 只允许使用原生上游流式、原生 tool loop 的 conversation driver；prompt-style generation 则走独立 generation runtime。
...
flowchart TD
    ...
    G --> H["ConversationDriver / GenerationDriver"]
...
- runtime driver 抽象：`crates/octopus-runtime-adapter/src/model_runtime/conversation_driver.rs`
- generation driver 抽象：`crates/octopus-runtime-adapter/src/model_runtime/generation_driver.rs`
- driver registry 与协议族分发：`crates/octopus-runtime-adapter/src/model_runtime/driver_registry.rs`
- turn 主循环：`crates/octopus-runtime-adapter/src/agent_runtime_core.rs`
- 预算预留与结算：`crates/octopus-runtime-adapter/src/model_budget.rs`
...
- `ModelRegistryRecord`
  - 描述 model 归属 provider、推荐用途、`surface_bindings`、能力、context window、max output；每个 binding 都携带 `executionProfile`，用于声明该 surface 的真实执行类别、上游流式能力和 tool loop 真值。
- `ConfiguredModelRecord`
  - 描述某个“可被实际选择的模型实例”，包含 `configured_model_id`、显示名、凭据引用、base URL、`budgetPolicy`、`tokenUsage`、状态。
- `CredentialBinding`
...
- `ResolvedExecutionTarget`
  - runtime 真正执行时使用的目标，包含 provider、model、surface、protocol family、credential、base URL、capabilities、max output、execution profile 与预算策略。
...
`registry.resolve_target(configured_model_id, preferred_surface, required_execution_class)` 的解析规则现在围绕真实执行类别展开：

1. 校验该 configured model 是否在当前 project 允许范围内。
2. 找到 `ConfiguredModelRecord`，再找到对应 `ModelRegistryRecord` 与 `ProviderRegistryRecord`。
3. 如果调用方声明了 `required_execution_class`，只保留 `executionProfile.executionClass` 匹配的 `surface_binding`；agent session 路径必须要求 `agent_conversation`。
4. 优先按 `preferred_surface` 选 model 的 `surface_binding`，否则选同 execution class 下第一个启用的 binding。
5. 在 provider surfaces 中优先匹配同 surface + 同 protocol family，否则退化到同 surface。
6. 生成 `ResolvedExecutionTarget`，并把 surface/profile/budget policy 一起带入执行阶段。
...
`execution_target.rs` 会在真正执行前补一层 secret hydrate 与执行前预算校验：

- 如果 `credential_ref` 是 `secret-ref:*`，会先解析成真实 secret。
- hydrate 失败会直接阻断执行。
- budget policy 只负责声明预算总量、accounting mode、traffic classes、warning thresholds 与 reservation strategy；真正的预算预留与结算由 `model_budget.rs` 执行。
...
5. 模型执行：
   - agent session 走 `RuntimeAdapter::execute_resolved_conversation()`
   - generation 调用走独立 `execute_generation()` 路径
   - `driver_registry` 按 `protocol_family + execution class` 选择具体 driver
...
- agent session 不再允许从 conversation 路径回退到 `simple_completion`。
- 预算治理不再依赖“调用结束后再看令牌上限是否超额”，而是先预留、后结算。
...
| Protocol family | 执行类别 | 说明 |
| --- | --- | --- |
| `anthropic_messages` | `agent_conversation` | 上游流式 + 原生 tool loop，当前最完整路径之一 |
| `openai_chat` | `agent_conversation` | 上游流式 + 原生 tool loop，当前最完整路径之一 |
| `openai_responses` | `single_shot_generation` | 保留 generation surface，不再作为 agent session driver |
| `gemini_native` | `single_shot_generation` | 当前只进入 generation runtime，不参与 agent session |
| `vendor_native` | `unsupported` | 注册表可声明，但当前 runtime 不可执行 |
| `realtime` | `unsupported` | 注册表可声明，但当前 batch 不实现 realtime 执行链路 |
...
这些能力在某些 provider/model 上已经被声明，但是否能在 Octopus 的 session/tool loop 中实际工作，要以 `executionProfile` 与 driver registry 当前实现为准。
...
- `budgetPolicy.totalBudgetTokens`: `1000000`
- `budgetPolicy.accountingMode`: `provider_reported`
- `budgetPolicy.reservationStrategy`: `fixed`
...
5. 即使某个 configured model 已经存在，只要它的 `executionProfile.executionClass` 不是 `agent_conversation`，它也不会出现在会话模型选择器里。
...
- 如果 session policy 最终仍未绑定模型，执行目标解析阶段才会回退到 registry 的 `conversation` 默认选择，但仍要求该目标具备 `agent_conversation` 执行类别。
...
2. 解析为 `ResolvedExecutionTarget`，并要求 `executionProfile.executionClass = agent_conversation`。
3. hydrate secret / 预留预算。
4. 构造 `RuntimeConversationRequest`。
5. conversation driver 按 `protocol_family` 发起上游流式请求。
6. runtime 逐步消费 `model.started / model.delta / model.tool_use / model.usage / model.completed` 事件，并在结束后结算预算。
7. 把 `AssistantEvent` 回流为 runtime events，再同步到前端。
...
这意味着 catalog 展示能力已经跑在实现前面，后续需要把 responses / gemini / vendor native / realtime 的 runtime 链路补齐；在那之前，UI 必须区分“可展示”和“可用于 agent session”。
