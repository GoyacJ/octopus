...
| 6 | 2026-04-22 | `07-hooks-lifecycle.md` §7.6 / §7.11, `docs/plans/sdk/02-crate-topology.md` §2.9, `crates/octopus-sdk-hooks/src/runner.rs` | 规范与拓扑文档都要求 hook source order 为 `session > project > plugin > workspace > defaults`，但当前 hooks crate 实现仍把 `plugin` 排在最前、`session` 排在最后。若直接推进 W6 plugin hook 接线，会把错误重写/拦截顺序固化进 Brain Loop。 | 以规范层与 `02` 控制面为准：source order 固定为 `session > project > plugin > workspace > defaults`。本次回写 `07` 章消除 plugin 插入位的歧义；实现侧必须在 W6 plugin hook 接线前校回该顺序，否则视为阻塞，不允许继续。 | `main` working tree |
| 7 | 2026-04-23 | `08-long-horizon.md` §8.9, `docs/plans/sdk/14-residual-capability-closure.md` Task 7 | `08` 章把每次 Coding Agent session 启动前的恢复步骤写成“读取 `NOTES.md` / `runtime/notes/<session>.md` / last 5 commits，再发一条 `context_restored`”。当前 Task 7 runtime 实现只保证恢复 `NOTES.md`、`runtime/notes/<session>.md`、`runtime/todos/<session>.json`，并写入 `context_restored` render/restore message；没有内建 `git log` / last 5 commits 读取。 | 以 Task 7 live runtime 为准：session 恢复合同当前只覆盖 `NOTES.md`、`runtime/notes/<session>.md`、`runtime/todos/<session>.json` 与 `context_restored`。Git history 仍可作为 harness 或人工 preflight，但不属于当前 runtime 的内建恢复保证。后续若要把 commit 恢复纳入 live path，必须与实现和测试同批补齐。 | `goya/sdk-task2-residual-closure` working tree |

---

**最后更新**：2026-04-22 · 所有设计决策均对应 `references.md` 中的一级来源条目；源码引用以 `restored-src v2.1.88` 快照为准。
