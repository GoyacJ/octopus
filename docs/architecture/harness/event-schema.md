...
所有 Event 统一为一个 enum（`harness-contracts::Event`）。分类如下：

| 分类 | 代表变体 |
|---|---|
| **Session 生命周期** | `SessionCreated / SessionForked / SessionEnded / SessionReloadApplied` |
| **Run 执行** | `RunStarted / RunEnded` |
| **消息流** | `UserMessageAppended / AssistantDeltaProduced / AssistantMessageCompleted` |
| **工具调用** | `ToolUseRequested / ToolUseApproved / ToolUseDenied / ToolUseCompleted / ToolUseFailed` |
...
```rust
let base = SqliteEventStore::open("data/main.db").await?;
...
- 多存储后端组合（如 `SqliteEventStore + JsonlEventStore` 双写）由业务层通过 `CompositeEventStore` 实现
...
SessionReloadApplied
```

这些事件决定业务语义正确性（审批、生命周期、安全事件），任何 OverflowPolicy 都**不得**丢弃它们；若 buffer 满则 `BlockProducer`。

**默认可丢的事件候选**：`AssistantDeltaProduced / UsageAccumulated / TraceSpanCompleted / SteeringMessageQueued / SteeringMessageApplied / SteeringMessageDropped / ExecuteCodeStepInvoked`。Steering 与 PTC 步级事件高频，业务可按需采样；安全开关型 `ExecuteCodeWhitelistExtended` 与审批型 `SteeringMessageDropped { reason: PluginDeny }` 仍走 never-drop（后者由 `SessionEventSinkPolicy::sticky_filter` 选择性提升）。业务层按需加入 `lossy_event_kinds`。
