...
文档状态：Draft  
文档定位：仓库级研发流程基线  
关联文档：`docs/PRD.md`、`docs/SAD.md`、`docs/DEVELOPMENT_STANDARDS.md`、`docs/VISUAL_FRAMEWORK.md`
...
4. 安全底线与 MVP 迭代并行推进，不允许用“先跑起来再说”替代治理。
...
3. 视觉框架规划：明确表面职责、导航、首批页面、主题和语言边界。
...
截至 2026-03-24，`octopus` 当前处于“治理基线已锁定、基础骨架初始化中”的阶段。
...
1. 补齐 VibeCoding 基线与视觉框架文档。
2. 建立 ADR / plans / PR 模板 / hooks / CI 入口。
3. 初始化 monorepo 根配置与目录骨架。

当前不做：

1. 大面积业务功能代码铺设。
2. 未经验证的第二套框架或替代技术栈引入。
3. 绕过 ask-user / approval / audit / resume 主线直接扩展复杂能力。
...
4. 最小 Web 控制面壳。
5. 最小 Rust 控制面与运行时骨架。
6. 首条 `run -> interaction/approval -> resume -> timeline/audit` MVP 纵切片。
7. 后续能力按验证结果逐条解锁。
