...
> 关键交付：tool / hook / context / session 四 crate 完整 + 正式 `Session::run_turn` 跑通最小 E2E
...
2. **本里程碑结束 = 最小可运行 SDK**：通过正式 `Session::run_turn` 能跑 "create_session → run_turn → ListDir 工具调用 → 输出"
3. **M3 remediation 更新**：`tests/e2e_minimal.rs` 临时 driver 已删除；最小闭环由 `octopus-harness-session::turn` 承载，完整 engine 主循环仍归 M5
...
**下一步**：M3-T20 · M3 E2E Gate 检查。

---

### M3-T20 · M3 E2E Gate 检查

**预期产物**：
- `crates/octopus-harness-session/tests/run_turn.rs`：正式 `Session::run_turn` 最小闭环验证（不是 M5 的完整 engine 主循环）
...
**预期产物**：
- `crates/octopus-cli/src/run_once.rs`：把 `run --once` 的旧 SDK 调用路径替换为 M3 正式 `Session::run_turn` 调用：
  ```rust
...
**提交结果**：
- `run --once <prompt>` 已接入 M3 `Session::run_turn` runtime，使用 `SessionOptions.workspace_root` 从 CLI 当前工作目录派生路径
...
|---|---|
| **状态** | 待派发 |
| **依赖** | M3-T20（正式 `Session::run_turn` 最小闭环完成） |
| **预期 diff** | < 250 行 |
| **预期工时** | AI 1.5h + 人类评审 30min |
...
- `crates/octopus-harness-session/tests/spike_steering.rs`：
  ...
  - L5 TTL 过期
  - 由于 M3 时还没完整 engine，本 spike 用 mock LLM + M3-T20 `Session::run_turn` runtime 跑
