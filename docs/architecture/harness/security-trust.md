...
```text
Tool::execute("bash", { command: "rm -rf ~" })
    │
    ▼
...
| Model Credential Pool | **默认强隔离** | `CredentialKey.tenant_id` 必填且 `ban_list / cooldown` 按整键分桶，跨租户不共享；显式跨租户共享必须使用 `TenantId::SHARED` 哨兵并触发 `Event::CredentialPoolSharedAcrossTenants` 审计事件（详见 `crates/harness-model.md §2.4`） |
...
- 所有 `HookFailedEvent`（不论 `failure_mode` 为 `FailOpen` 或 `FailClosed`，详见 §10.W）
- 所有 `CredentialPoolSharedAcrossTenants`（凭证池首次解析到 `TenantId::SHARED` 时触发，详见 `crates/harness-model.md §2.4`）
...
| 密钥不应进 `main.db` | `data/main.db` 只保存 projection / index / hash / metadata；Redactor 在 Event 写入前剥离敏感字段 |
| `runtime/events/*.jsonl` 审计流 | Octopus 产品基线使用 `JsonlEventStore` 落盘，作为事件真相源 |
...
| 风险 | 场景 | 缓解 |
|---|---|---|
| **权限规避** | 攻击者通过 prompt injection 诱导模型 `tool_search` 把高危工具 materialize 进来再调用 | 权限决策链路不变：materialize 只让 schema 可见，实际 execute 仍走 `PermissionBroker`（ADR-007）；`DeferPolicy` 不替代 `PermissionMode` |
...
| 嵌入工具被滥用为写 / 网络通道 | `EmbeddedToolWhitelist` 默认仅 7 个 read-only built-in；扩展要求 `is_destructive == false && is_read_only == true`，且来源为 built-in 或 admin-trusted plugin；`Builtin` 是 `ToolOrigin`，不是 `TrustLevel` |
...
## 10.W Hook 层 Fail-Open 受控例外（P6 边界澄清）

`overview.md §2 P6` 规定本 SDK 默认 Fail-Closed：Tool 默认不并发安全、不只读、破坏性未知；Permission Broker 默认 Deny-All。**Hook 层是 P6 的受控例外**——`HookFailureMode` 默认 `FailOpen`（`crates/harness-hook.md §2.6.1`）。本节集中说明边界与缓解。

### 10.W.1 例外原因

- **职责定位**：Hook 是横切观察/补强插件，不是主路决策者。Permission/Tool 层已是 Fail-Closed 的最后防线；若 Hook 故障即拒，会让无关的 audit/observability hook 阻塞主流程，**降低整体可用性而不提高安全性**。
- **可信来源不对称**：Bundled / Plugin{AdminTrusted} 可显式声明 `FailClosed`；UserControlled / User / Workspace 来源的 hook 强制 `FailOpen`，避免外部用户用一个故意失败的 hook 把会话挂死。

### 10.W.2 缓解措施（不可禁用）

| 措施 | 来源 | 说明 |
|---|---|---|
| 失败必发事件 | `harness-hook.md §2.6.1` | 无论 mode，`HookFailedEvent` 都必记，审计可见（必记事件，列入 §8.1） |
| Admin 域可强制 FailClosed | `harness-hook.md §2.6.1` | 安全/合规类 hook（DLP、PermissionAudit）可声明 `FailClosed`，失败即终止 dispatch + `ToolUseDenied { reason: HookFailClosed }` |
| 内置审计 hook 强制 FailClosed | `harness-hook.md §2.6.1` 表 | PermissionAudit 等内置审计 hook 默认 `FailClosed`，否则审计漏写即视为高危 |
| User 域不得越权 | ADR-006 + 注册期校验 | `Plugin{UserControlled}` 声明 `FailClosed` 时由 `PluginRegistry::validate()` 阶段拒绝 |
| 事务半成品不外泄 | `harness-hook.md §2.6.2` | PreToolUse 链 all-or-nothing：任一 hook 失败按 mode 决定走向，但已改写的 input / additional_context 不外泄到主流程 |

### 10.W.3 治理要求

- 任何 PR 想把 Hook 默认改为 `FailClosed` 或弱化上述缓解，必须先开 ADR 论证（参考 ADR-006 / ADR-007 的论证形式）。
- 业务方在审计日志中看到 `HookFailedEvent` 而 hook 未阻塞流程时，应理解为"由 P6 受控例外允许"，而非配置错误。
...
Accepted 基线要求如下。实现、发布评审不得把这些要求改写为 checklist 状态项。

| 断言 | 归属 |
|---|---|
| 所有公开 API 必须有 `#[non_exhaustive]` 标注，允许未来扩展。 | API 设计评审 |
| 所有凭证字段必须使用 `SecretString` 或等价的 secret wrapper。 | Contract / Security |
| `Debug` / `Display` 实现不得泄漏密文。 | Code Review / Security |
| 默认配置必须能通过安全审计指令集；失败即阻断发布。 | Release Gate |
| 至少一个 CI 任务必须运行 `cargo-audit`。 | CI / Release Gate |
| 所有 `rg` 命中 "password" / "secret" 字样的字段必须有 redact 处理或明确的 false-positive 记录。 | Security Review |
