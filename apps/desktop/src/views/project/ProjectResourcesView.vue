...
const canManageResources = computed(() =>
  workbench.hasPermission('project:resource:update', 'update', 'project', workbench.currentProjectId),
)
  ...
  <div class="w-full flex flex-col gap-6 pb-20 h-full min-h-0">
    ...
    <div data-testid="resources-toolbar" class="px-2 flex flex-wrap items-center justify-between gap-4 border-b border-border-subtle pb-4">
      ...
      <div class="flex items-center gap-2">
        ...
        <div v-if="canManageResources" class="relative">
          ...
          <div v-for="resource in pagedItems" :key="resource.id" :data-testid="`resource-item-${resource.id}`">
            ...
            <template #actions>
              <div class="flex items-center gap-1">
                ...
                <UiButton v-if="canManageResources" :data-testid="`resource-edit-${resource.id}`" variant="ghost" size="icon" class="h-7 w-7" @click.stop="openEdit(resource.id)">
                  <Pencil :size="14" />
                </UiButton>
                <UiButton v-if="canManageResources" :data-testid="`resource-delete-${resource.id}`" variant="ghost" size="icon" class="h-7 w-7 text-destructive hover:bg-destructive/10" @click.stop="openDelete(resource.id)">
                  <Trash2 :size="14" />
                </UiButton>
              </div>
            </template>
          ...
          >
            ...
            <template #actions>
              <div class="flex items-center gap-1">
                ...
                <UiButton v-if="canManageResources" :data-testid="`resource-edit-${resource.id}`" variant="ghost" size="icon" class="h-6 w-6" @click.stop="openEdit(resource.id)">
                  <Pencil :size="12" />
                </UiButton>
                <UiButton v-if="canManageResources" :data-testid="`resource-delete-${resource.id}`" variant="ghost" size="icon" class="h-6 w-6 text-destructive hover:bg-destructive/10" @click.stop="openDelete(resource.id)">
                  <Trash2 :size="12" />
                </UiButton>
              </div>
            </template>
