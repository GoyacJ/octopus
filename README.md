...
This repository now proves a **local, test-verified GA runtime baseline through Slice 8 trigger expansion plus the minimum `desktop + remote-hub + schema-ts + hub-client` surface foundation**. The tracked implementation covers the local SQLite-backed governed loop for `Automation(manual_event | cron | webhook | mcp_event) -> TriggerDelivery -> Task -> Policy / Budget / Approval -> Run -> Execution Adapter / MCP Gateway -> EnvironmentLease -> Artifact -> KnowledgeCandidate gate -> Shared Knowledge recall -> Audit / Trace`, plus a thin remote-hub webhook ingress / cron tick shell and the first desktop and shared TypeScript consumer surfaces. It does **not** yet prove real credentialed MCP transport, richer remote-hub persistence / auth, a minimum automation-management surface, approval-driven knowledge promotion, vector retrieval, Org Knowledge Graph promotion, or a full GA implementation tree.  
