...
### 7.10 `minimal entrypoints`（Task 8 audit：`retired` / `hide from live`）

> Task 8 收口结果：`RuntimeSdkDeps::minimal(...)` 已删除；CLI session boot 不再写 `octopus-cli:minimal`；crate-level minimal scaffold 注记已移除。生产主路径现在只承认 `RuntimeSdkFactory::build_live(...)`、显式 `RuntimeSdkDeps { ... }` 组装，以及各 crate 的正式 public contract。

| 旧入口 / 注记 | 主要符号 | 替代位置 | 迁移周 | 状态 |
|---|---|---|---|---|
| `crates/octopus-platform/src/runtime_sdk/builder.rs` | `RuntimeSdkDeps::minimal(...)` | `octopus-platform::runtime_sdk::RuntimeSdkFactory::build_live(...)` + 显式 `RuntimeSdkDeps` 组装 | Post-W8 | retired |
| `crates/octopus-cli/src/run_once.rs` | `DEFAULT_CONFIG_SNAPSHOT_ID = "octopus-cli:minimal"` | `octopus-cli::run_once` 的 `octopus-cli:local-run` snapshot id 与 real runtime boot 路径 | Post-W8 | retired |
| `crates/octopus-sdk-contracts/src/lib.rs` | `W1 Task 1 intentionally keeps this crate as a minimal scaffold` 注记 | `docs/plans/sdk/02-crate-topology.md §2.1` 的正式公共面合同 | Post-W8 | removed |
| `crates/octopus-sdk-session/src/lib.rs` | `W1 Task 1 intentionally keeps this crate as a minimal scaffold` 注记 | `docs/plans/sdk/02-crate-topology.md §2.2` 的正式 store / replay 合同 | Post-W8 | removed |
| `crates/octopus-sdk-model/src/lib.rs` | `W2 Task 1 intentionally keeps this crate as a minimal scaffold` 注记 | `docs/plans/sdk/02-crate-topology.md §2.3` 的正式 model/catalog/router 合同 | Post-W8 | removed |

### 7.11 `session compatibility`（Task 1 freeze：`supported compat`）

> Task 8 审计结果：这些路径继续保留，但 owner 被收窄到 `octopus-sdk-session` 的旧数据恢复边界。现有验证点：`sqlite_jsonl.rs` 覆盖 legacy event-only JSONL、legacy wrapper、legacy runtime envelope skip；`sqlite/schema.rs` 覆盖 legacy SQLite table rename 且证明不会误碰 auth `sessions` 表。
...
> Task 8 审计结果：`shim_tool_context()` 只允许留在 SDK MCP directory transport 兼容调用链，不能回流到 live runtime；hidden builtin metadata 只允许通过 `configuredModels` unsupported projection 暴露，默认 live catalog / default selections 一律跳过。

| shim 路径 | 主要符号 | 替代位置 | 迁移周 | 状态 |
|---|---|---|---|---|
| `crates/octopus-sdk-tools/src/registry.rs` | `shim_tool_context()` | `octopus-sdk-core::tool_dispatch` 驱动的 runtime-owned `ToolContext`；当前只服务 `SdkTransport::call_tool(...)` compat shim | Post-W8 | hide from live |
...
| 2026-04-23 | Task 8 收口：删除未使用的 `RuntimeSdkDeps::minimal(...)`、把 CLI snapshot id 从 `octopus-cli:minimal` 改为 `octopus-cli:local-run`、移除 crate-level minimal scaffold 注记，并把 `session compatibility` / `runtime shims` 的保留边界补成显式审计说明。 | Codex |
