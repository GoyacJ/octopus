...
## Checkpoint 2026-04-19 15:53

- Batch: Mainline merge resolution and verification
- Completed:
  - resolved the `main` merge conflicts in `crates/octopus-server/src/workspace_runtime.rs` by preserving both the runtime-generation coverage from `main` and the workspace/project governance tests from the feature branch
  - regenerated `packages/schema/src/generated.ts` from the merged OpenAPI sources instead of hand-editing generated output
  - re-ran the full merge verification on `main` and confirmed the earlier desktop timeouts were caused by concurrent Rust test load rather than a functional regression
- Verification:
  - `pnpm openapi:bundle` -> pass
  - `pnpm schema:generate` -> pass
  - `pnpm schema:check` -> pass
...
- Blockers:
  - none
- Next:
  - create the merge commit on `main`, then remove the feature worktree and branch
