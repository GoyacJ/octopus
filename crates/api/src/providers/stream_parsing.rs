...
use super::response_normalization::{normalize_finish_reason, OpenAiUsage};
...
impl OpenAiSseParser {
    pub(crate) fn with_context(provider: impl Into<String>, model: impl Into<String>) -> Self {
        Self {
            buffer: Vec::new(),
    ...
    pub(crate) fn push(&mut self, chunk: &[u8]) -> Result<Vec<ChatCompletionChunk>, ApiError> {
...
impl StreamState {
    ...
    pub(crate) fn finish(&mut self) -> Result<Vec<StreamEvent>, ApiError> {
        if self.finished {
            return Ok(Vec::new());
        }
        self.finished = true;
