...
|---|---|---|---|---|---|
| `octopus-harness-model` | Features match D10: provider features, `all-providers`, `mock`; depends on `contracts`, `async-trait`, `serde`, `thiserror`, `tokio`. | T01 starts trait/types; T02+ add Anthropic; T04.5+ add provider implementations. | `harness-model.md` also lists `rate-limit-observer`, `cassette`, `oauth`, `redactor`, `otel`, `http-client`, `openai-compatible`, but D10 does not list those internal features. | Only `contracts` is allowed by default. `model -> observability` is registered as `redactor` exception in D2 §10, but M2 says not to implement it. | `OK` for T01. `SPEC-CLARIFY-REQUIRED` before adding non-D10 model features. `DEFER-M2-T02/T04.5+` for provider deps. |
...
|---|---|
| M2-T02/T04.5+ | Decide whether `harness-model.md` internal features are canonical or should be folded behind D10 provider features. |
